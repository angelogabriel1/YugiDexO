...
try {
  const [profiles, cards, legacyAccounts, deckTables] = await Promise.all([
    ...
    pool.query("select to_regclass('public.decks') is not null as decks, to_regclass('public.deck_cards') is not null as deck_cards")
  ]);
  const [decks, deckCards] = deckTables.rows[0].decks && deckTables.rows[0].deck_cards
    ? await Promise.all([
  ...
  console.log(JSON.stringify({
    ...
    decks: decks.rowCount, deckCards: deckCards.rowCount, legacyAccounts: legacyAccounts.rowCount
  }));
} finally {
  await pool.end();
}
